...
function Cart(){
    ...
    const getProducts=async()=>{
        try{
              if(!userId){
             let message='Please login to view cart.'
               setStatus(401);
        setMsg(message);
        return
            }
            ...
            console.log(data)
            const allProducts=await (await fetch('http://localhost:3000/products')).json()
            const productIDs=data.map(item=>item.productId)
            const quants=data.map(item=>item.quantity)
            var filteredProducts = allProducts.filter(product => productIDs.includes(product.product_id));
            filteredProducts=filteredProducts.map((item,index)=>({
                ...item,quantity:quants[index]
            }))
